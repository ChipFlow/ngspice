...
# Make sure we have the right brew environment
eval $(brew shellenv)
echo "HOMEBREW_PREFIX=$HOMEBREW_PREFIX"
HOMEBREW_NO_AUTO_UPDATE=1 brew install -q libx11 libxaw bison flex libtool autoconf automake libomp readline $HOMEBREW_EXTRA
...
export CFLAGS="-m64 -O0 -g -Wall -Wno-unused-but-set-variable -I$HOMEBREW_PREFIX/include $READLINE_CFLAGS $FREETYPE2_CFLAGS $X11_CFLAGS -I$HOMEBREW_PREFIX/opt/libomp/include/"
export LDFLAGS="-m64 -g -L$HOMEBREW_PREFIX/lib $READLINE_LIBS $FREETYPE2_LIBS $X11_LIBS -L$HOMEBREW_PREFIX/opt/libomp/lib -lomp"
...
if [ ! -d _build ]; then
  mkdir -p _build
  if [ $? -ne 0 ]; then  echo "mkdir _build failed"; exit 1 ; fi
fi
